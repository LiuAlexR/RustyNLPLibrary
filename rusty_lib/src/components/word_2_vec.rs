        ...
        for i in self.window_size..(corpus.len() - self.window_size) {
            ...
            for j in (1..=(self.window_size)).rev() {
                ...
                for _ in 0..self.k {
                    ...
                    // Adjust c_neg weight
                    let delta_c = sigmoid(dot_product(&self.context_slice[c_neg], &self.target_slice[target]));
                    let new_c_neg = vector_diff(&self.context_slice[c_neg], &vector_x_constant( &self.target_slice[target], self.learning_rate * delta_c));
                    self.context_slice[c_neg] = new_c_neg;
                    
                    // Add to negative gradient
                    negative_gradient = vector_sum(&negative_gradient, &vector_x_constant(&self.context_slice[c_neg], delta_c));
                }
...
pub fn build_model<'a> (vocab: &'a [String], dimension: usize, window_size_: usize, num_of_negs: usize, learning_rate_: f64) -> Word2Vec<'a> {
    let range = Uniform::new(-1.0, 1.0).expect("Failed");
    let mut rng = rand::rng();
    let vocab_size = vocab.len();
    ...
    Word2Vec {
        vocabulary: vocab,
        target_matrix: create_random_matrix(),
        context_matrix: create_random_matrix(),
